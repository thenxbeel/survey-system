// Module: Reports & Export Center — Types, constants & template metadata.
//
// LIVE DATA VERSION — the mockSavedReports and mockScheduledReports arrays
// have been removed. All report downloads now go through /api/reports/export
// (Prisma-backed). This file is kept ONLY to preserve the types, the
// REPORT_TYPES / REPORT_FORMATS / SCHEDULE_FREQS / PERIODS constants, the
// REPORT_TEMPLATES metadata, the computeReportStats helper, and the static
// chart preview data used by the ReportPreview component.
...
export type ReportFormat = 'pdf' | 'excel' | 'csv' | 'pptx'
...
export const REPORT_FORMATS: { value: ReportFormat; label: string; icon: string; color: string }[] = [
  ...
  { value: 'pptx',  label: 'PowerPoint',  icon: 'Presentation',    color: '#F5A623' },
]
