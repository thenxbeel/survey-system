// Module: User Management — Types, constants & helper functions.
//
// LIVE DATA VERSION — the mockUsers array has been removed. All user data now
// comes from /api/users (Prisma-backed). This file is kept ONLY to preserve
// the AppUser / UserStats / UserFilters types, the ROLES / DEPARTMENTS /
// BRANCHES constants, the ROLE_PERMISSIONS / ROLE_META metadata, and the
// computeUserStats / applyUserFilters / applyUserSort helpers that the
// existing UI components depend on.
...
export function applyUserFilters(users: AppUser[], f: UserFilters): AppUser[] {
  ...
  return users.filter(u => {
    if (term) {
      const matches = u.name.toLowerCase().includes(term) || u.email.toLowerCase().includes(term) || u.employeeId.toLowerCase().includes(term)
...
export function applyUserSort(users: AppUser[], key: UserSortKey, dir: 'asc' | 'desc'): AppUser[] {
  return [...users].sort((a, b) => {
    ...
    switch (key) {
      ...
      case 'role':          cmp = roleRank[a.role] - roleRank[b.role]; break
      case 'lastLogin':
