...
interface HeatmapEntry { y: string; x: string; value: number }
...
export function ActivityHeatmap({ metric, groupBy, filterOverride }: ChartProps) {
  ...
  useEffect(() => {
    ...
    params.set('groupBy', groupBy || 'date')
    params.set('metric', metric || 'responses')
  ...
  if (!data) return <div className="flex h-full items-center justify-center text-[12px]" style={{ color: 'var(--text-muted)' }}>No data available</div>

  const { yLabels, xLabels, grid, groupBy: serverGroupBy } = data
  
  const valueMap = new Map<string, number>()
  for (const entry of grid) {
    valueMap.set(`${entry.y}-${entry.x}`, entry.value)
  }

  const isDate = serverGroupBy === 'date'

  return (
    <div className="flex h-full flex-col gap-2.5">
      <div className="flex flex-1 gap-2.5">
        {/* Y-axis labels */}
        <div className="flex w-16 flex-col justify-around text-[9px] font-medium text-[#8A94A6]">
          {yLabels.map((y) => <span key={y} className="truncate text-right" title={y}>{y}</span>)}
        </div>

        {/* Grid */}
        <div className={`grid flex-1 grid-flow-col gap-[3px]`} style={{ gridTemplateRows: `repeat(${yLabels.length}, minmax(0, 1fr))` }}>
          {xLabels.map((x, xIdx) =>
            yLabels.map((y, yIdx) => {
              const value = valueMap.get(`${y}-${x}`) ?? 0
              const label = isDate ? `${y} ${x.padStart(2, '0')}:00` : `${y} on ${x}`
              return (
                <div
                  key={`${yIdx}-${xIdx}`}
      ...
      {/* X-axis labels */}
      {isDate ? (
        <div className="ml-16 flex justify-between text-[9px] text-[#8A94A6]">
          {[0, 6, 12, 18, 23].map(h => (
