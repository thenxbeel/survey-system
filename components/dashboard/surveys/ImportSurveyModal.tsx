...
export function ImportSurveyModal({ open, onClose }: Props) {
  ...
  function validate(raw: string): { ok: true; record: SurveyRecord } | { ok: false; error: string } {
    ...
    const record: SurveyRecord = {
      ...
      department:    typeof obj.department === 'string' ? obj.department : settingsState.profile.department,
      ...
      try {
        const res = await fetch('/api/surveys', {
          ...
          body: JSON.stringify({
            ...
            department: result.record.department || undefined,
            branch: result.record.branch || undefined,
